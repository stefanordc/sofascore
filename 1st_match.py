...
if __name__ == "__main__":
    ...
    # Verifica se já existe o id_jogo no banco de dados
    cursor.execute("SELECT COUNT(*) FROM 1_tempo WHERE id_jogo = %s", (id_jogo,))
    ...
    if exists == 0:
        ...
        df_1st = extract_statistics(stats_data, "1ST", id_jogo, home_club, away_club)

        # Inserir dados no banco com proteção de nomes das colunas
        cols = ','.join([f"`{col}`" for col in df_1st.columns])
        vals = ','.join(['%s'] * len(df_1st.columns))
        insert_query = f"INSERT INTO 1_tempo ({cols}) VALUES ({vals})"

        for i in range(len(df_1st)):
            cursor.execute(insert_query, tuple(df_1st.iloc[i]))
        conn.commit()
