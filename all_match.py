...
if __name__ == "__main__":
    ...
    cursor.execute("SELECT COUNT(*) FROM estatisticas_partidas WHERE id_jogo = %s", (id_jogo,))
    ...
    if exists == 0:
        ...
        df_all = extract_statistics(stats_data, "ALL", id_jogo, home_club, away_club)

        cursor.execute("SHOW COLUMNS FROM estatisticas_partidas")
        existing_columns = set(row[0] for row in cursor.fetchall())
        df_columns = set(df_all.columns)

        missing_cols = existing_columns - df_columns
        for col in missing_cols:
            df_all[col] = None

        df_all = df_all[list(existing_columns)]

        cols = ','.join([f"`{col}`" for col in df_all.columns])
        vals = ','.join(['%s'] * len(df_all.columns))
        insert_query = f"INSERT INTO estatisticas_partidas ({cols}) VALUES ({vals})"

        for i in range(len(df_all)):
            cursor.execute(insert_query, tuple(df_all.iloc[i]))
        conn.commit()
